...
func main() {
	...
	for _, f := range filterednames {
		...
		if err == nil {
			...
			wgdate.Add(1)
			workers <- true
			go func(wgdate *sync.WaitGroup, sdswift *gostardustswift.StarDustSwift, curipinfo *IPInfo, f string) {
				processfile(wgdate, sdswift, curipinfo, f)
				<-workers
			}(&wgdate, sdswift, curipinfo, f)
...
func processfile(wgdate *sync.WaitGroup, sdswift *gostardustswift.StarDustSwift, ipinfo *IPInfo, filepath string) {
	...
	cnt := 1
	allinfo := make([]*greynetanalysis.PacketSummary, 0)
	infochan := make(chan *greynetanalysis.PacketSummary, 1000)
	wgchan.Add(1)
	go func() {
	...
	for {
		...
		if err == nil {
			...
			wg.Add(1)
			go func(pkt gopacket.Packet, cnt int) {
				annotation := greynetanalysis.AnnotatePacketwithSummary(pkt, cnt, ipinfo.Pfx2asn, ipinfo.Mmgeo, ipinfo.Naqgeo, ipinfo.KnownScanner)
				infochan <- annotation
				wg.Done()
			}(pkt, cnt)
			cnt += 1
	...
	gzwrite.Flush()
	gzwrite.Close()
	_, err = sdswift.PutGreynetSummary(jts, jname, &bjson)
	if err != nil {
